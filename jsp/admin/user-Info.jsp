                    ...
                    <div class="dashboard-wraper">
                        ...
                        <table class="property-table-wrap responsive-table">
                            ...
                            <c:if test="${Info.userName!=null&&Info.userName!=''}">
                                <tr>
                                    <td>
                                        <span title="用户名">${Info.userName}</span> <span title="姓名">${c.userDisplayName}</span> <br><span title="身份证号码">${c.idCard}</span>
                                    </td>
                                    <td>
                                        <span title="邮箱">${Info.email}</span> <br>
                                        <span title="电话">${Info.phone}</span> <br>
                                    </td>
                                    <td>
                                        <span title="爱好">${Info.hobby}</span> <br>
                                        <span title="职业">${Info.job}</span>
                                    </td>
                                    <td>
                                        <span title="角色">
                                            <c:choose>
                                                <c:when test="${Info.role=='admin'}">
                                                    <span class="text-danger">管理员</span>
                                                </c:when>
                                                <c:when test="${Info.role=='owner'}">
                                        ...
                                        <c:choose>
                                            <c:when test="${Info.status == 0}">
                                                <a href="javascript:void(0)" class="delete"
                                                   onclick="disableUser(${Info.id})">禁用</a>
                                            </c:when>
                                            <c:otherwise>
                                                <a href="javascript:void(0)" class="delete"
                                                   onclick="enableUser(${Info.id})">启用</a>
                                            </c:otherwise>
                                        </c:choose>
                                    </td>
                                </tr>
